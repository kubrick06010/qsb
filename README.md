...
For LP and MIP models, plus minimum-cost flow, transportation, aggregate
planning, and goal programming, `--backend external` selects the optional
open-source HiGHS adapter when a host-provided `highs` executable is
discoverable through `QSB_HIGHS_PATH` or `PATH`. QSB does not bundle solver
binaries; see [docs/BACKENDS.md](docs/BACKENDS.md) for setup and scope.
